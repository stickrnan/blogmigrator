    ...
    public partial class MainWindow : Window
    {
        ...
        public void WriteXMLPosts()
        {
           ...
           try
           {
              ...
              foreach (
                  Post blogPost in App.sourceBlog.blogPosts)
              {
        ...
        public void ImportXMLPosts()
        {
           ...
           try
           {
              args.status = "Migrating posts from " + App.sourceBlog.serviceType +
                            " to " + App.destBlog.serviceType;
              migrationWorker.ReportProgress(0, args);
               ...
               for (int i = 0; i <= totalCount - 1; i++)
              {
                 ...
                 if (App.sourceBlog.postsToMigrate.Contains(Convert.ToInt32(pid)))
                 {
                    ...
                    if (currPost.categories != null)
                    {
                       ...
                       for (int j = 0; j <= currPost.categories.Length - 1; j++)
                       {
                          currCatRef = currPost.categories[j];
                           ...
                           bool isInt= Int32.TryParse(currCatRef.@ref , out categoryId);
                           string categoryName;
                           if (isInt)
                           {
                               categoryName = myGenerator.GetCategoryById(App.sourceBlog.blogData, categoryId);
                           }
                           else
                           {
                               categoryName = currCatRef.@ref;
                           }
                    ...
                    resultPost = blogService.InsertPost
                                               (App.destBlog.serviceUrl, App.destBlog.blogId,
                                                App.destBlog.username, App.destBlog.password,
                                                newPost, swLog,App.BatchMode);

                    swLog.WriteLine(newPost.postid + "," + newPost.link + "," +
                                    resultPost.postid + "," + resultPost.link);
              ...
              migrationWorker.ReportProgress(100, args);
           }
           catch (Exception ex)
           {
              swLog.Flush();
